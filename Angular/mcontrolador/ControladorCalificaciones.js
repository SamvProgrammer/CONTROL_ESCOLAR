(function() {
    angular.module("primariaAngular").controller("CalificacionCtrl", CalificacionController);
  
  function CalificacionController($auth, serviciomateria, $scope,Flash, serviciodocente, serviciogrupo, servicioevaluar ,serviciocurso, validaciones, toastr, servicioalugrupo) {
          var vm = this;
          
           //vm.getMaterias = getMaterias;
           //vm.verRegistro= verRegistro;
           // vm.getMateriasByAlumno=getMateriasByAlumno;
            vm.logueado = logueado;
            vm.getMateriasByDocente=getMateriasByDocente;
            vm.pdf = pdf;
            vm.preparaPDF = preparaPDF;
            vm.Init = Init;
            vm.valida=valida;
            vm.verCalifG1= verCalifG1;
            vm.obtenerEvaluaciones =obtenerEvaluaciones;
            vm.updateEvaluacionesAlumno=updateEvaluacionesAlumno;
            vm.valiFechasEval=valiFechasEval;
            vm.getFechasEvalBimestre=getFechasEvalBimestre;
            vm.updateFechaEvalBim=updateFechaEvalBim;
            vm.validaFechaNacimiento=validaFechaNacimiento;
            vm.pruebaValida = pruebaValida;
            vm.getFechasEval = getFechasEval;
          ...
          function Init() {
                ...
                $scope.$on('actualizame', getMateriasByDocente);
                getGrupoDocente();
                getCursoActual();
        ...
        function getGrupoDocente() {
                serviciogrupo.getGrupoDocente(local.id_user).then(function(data) {
                    ...
                    console.log("grupo del docente");
                    console.log(vm.datos_grupo_docente);
                    obtenerAlumnoEvaluar();
                }).catch(function(err) {});
            }
        ...
        function verCalifG1() {
            serviciomateria.verCalifG1(local.id_user).then(function(data) {
                vm.datos_g1 = data;
                console.log("calificaciones b1");
                console.log(vm.datos_g1);

                for(var i=0; i < vm.datos_g1.length; i++){
              if(vm.datos_g1[i].idBimestre === 1){
                vm.bim1= vm.datos_g1[i];
              }
              if(vm.datos_g1[i].idBimestre === 2){
                vm.bim2= vm.datos_g1[i];
              }
              if(vm.datos_g1[i].idBimestre === 3){
                vm.bim3= vm.datos_g1[i];
              }
              if(vm.datos_g1[i].idBimestre === 4){
                vm.bim4= vm.datos_g1[i];
              }
              if(vm.datos_g1[i].idBimestre === 5){
                vm.bim5= vm.datos_g1[i];
              }
            ...
            function AsignaridGrado1() {
              for(var i=0; i<vm.alumnoEvaluar.length; i++){
                


                  vm.alumnoEvaluar[i].idEA=vm.materiaDocente[0].idMateria;
                  vm.alumnoEvaluar[i].idEF=vm.materiaDocente[1].idMateria;
                  vm.alumnoEvaluar[i].idEsp=vm.materiaDocente[2].idMateria;
                  vm.alumnoEvaluar[i].idENS=vm.materiaDocente[3].idMateria;
                  vm.alumnoEvaluar[i].idFCE=vm.materiaDocente[4].idMateria;
                  vm.alumnoEvaluar[i].idMat=vm.materiaDocente[5].idMateria;
                
                
              }
              console.log(vm.alumnoEvaluar);
            }
        ...
        function getMateriasByDocente() {
            
            serviciomateria.getMateriasByDocente(local.id_user).then(function(data) {
                vm.materiaDocente = data;
                console.log("materias");
                console.log(vm.materiaDocente);
            })
        }
        ...
        function getgradoGrupoDocente() {
            ...
            serviciogrupo.getGrupoGradoDocente(local.id).then(function(data) {
...
function valida($event, num) {
          ...
          function obtenerAlumnoEvaluar() {
           
            servicioalugrupo.obtenerAlumnoEvaluar(vm.datos_grupo_docente[0].idGrupo).then(function(data) {
                vm.alumnoEvaluar = data;
                console.log(vm.alumnoEvaluar);
            })
        }
        ...
        function obtenerEvaluaciones() {
            ...
            servicioevaluar.getEvaluacionesD(local.id_user, vm.bim).then(function(data) {
        ...
        function obtenerEvaluaciones2() {
            console.log("entro a evaluacion");
            //valiFechasEval(1);
            
            servicioevaluar.getEvaluaciones(local.id_user, vm.bim).then(function(data) {
              vm.getEvaluaciones=data;
              console.log(vm.getEvaluaciones);
              servicioevaluar.getNumMaterias(vm.getEvaluaciones[0].idDocente, vm.getEvaluaciones[0].idAlumno).then(function(data) {
                vm.getNumMaterias=parseFloat(data[0].numMat) ;
        ...
        function valiFechasEval(bimestre){
          ...
          switch(bimestre){
            ...
            console.log("entró uno");
              fecha_inicio = new Date(vm.fechas_evaluaciones.fecha_inicio_b1.replace(new RegExp('-', 'g'), '/'));
              fecha_fin = new Date(vm.fechas_evaluaciones.fecha_fin_b1.replace(new RegExp('-', 'g'), '/'));
              salida = (((fecha_actual.getTime() >= fecha_inicio.getTime()) && (fecha_actual.getTime() <= fecha_fin.getTime())) ? true : false);
          ...
          console.log(salida);
            break;
            case "2":
            console.log("entró dos");
              fecha_inicio = new Date(vm.fechas_evaluaciones.fecha_inicio_b2.replace(new RegExp('-', 'g'), '/'));
              fecha_fin = new Date(vm.fechas_evaluaciones.fecha_fin_b2.replace(new RegExp('-', 'g'), '/'));
              salida = (((fecha_actual.getTime() >= fecha_inicio.getTime()) && (fecha_actual.getTime() <= fecha_fin.getTime())) ? true : false);
            console.log(fecha_inicio);
          console.log(fecha_fin);
          console.log(fecha_actual);
          console.log(salida);
            break;
            case "3":
            console.log("entró tres");
              fecha_inicio = new Date(vm.fechas_evaluaciones.fecha_inicio_b3.replace(new RegExp('-', 'g'), '/'));
              fecha_fin = new Date(vm.fechas_evaluaciones.fecha_fin_b3.replace(new RegExp('-', 'g'), '/'));
              salida = (((fecha_actual.getTime() >= fecha_inicio.getTime()) && (fecha_actual.getTime() <= fecha_fin.getTime())) ? true : false);
            console.log(fecha_inicio);
          console.log(fecha_fin);
          console.log(fecha_actual);
          console.log(salida);
            break;
            case "4":
            console.log("entró cuatro");
              fecha_inicio = new Date(vm.fechas_evaluaciones.fecha_inicio_b4.replace(new RegExp('-', 'g'), '/'));
              fecha_fin = new Date(vm.fechas_evaluaciones.fecha_fin_b4.replace(new RegExp('-', 'g'), '/'));
              salida = (((fecha_actual.getTime() >= fecha_inicio.getTime()) && (fecha_actual.getTime() <= fecha_fin.getTime())) ? true : false);
            console.log(fecha_inicio);
          console.log(fecha_fin);
          console.log(fecha_actual);
          console.log(salida);
            break;
            case "5":
            console.log("entró cinco");
              fecha_inicio = new Date(vm.fechas_evaluaciones.fecha_inicio_b5.replace(new RegExp('-', 'g'), '/'));
              fecha_fin = new Date(vm.fechas_evaluaciones.fecha_fin_b5.replace(new RegExp('-', 'g'), '/'));
              salida = (((fecha_actual.getTime() >= fecha_inicio.getTime()) && (fecha_actual.getTime() <= fecha_fin.getTime())) ? true : false);
            console.log(fecha_inicio);
          console.log(fecha_fin);
          console.log(fecha_actual);
        ...
        function getFechasEval() {
            ...
            servicioevaluar.getFechasEval().then(function(data) {
                vm.fechas_evaluaciones = data[0];
                console.log(vm.fechas_evaluaciones);
                //getFechasEvalBimestre(vm.bim);
                //console.log(valiFechasEval(1));
            })
        }

        function getFechasEvalBimestre(bimestre) {
           
            getFechasEval();
            switch(bimestre){
              case "1":
                  vm.fechaInicioG=moment(vm.fechas_evaluaciones.fecha_inicio_b1).toDate();
                  vm.fechaFinG=moment(vm.fechas_evaluaciones.fecha_fin_b1).toDate();
                  
                  console.log(bimestre);
              break;
              case "2":
                  vm.fechaInicioG= moment(vm.fechas_evaluaciones.fecha_inicio_b2).toDate();
                  vm.fechaFinG= moment(vm.fechas_evaluaciones.fecha_fin_b2).toDate();
                  
                  console.log(bimestre);
              break;
              case "3":
                  vm.fechaInicioG= moment(vm.fechas_evaluaciones.fecha_inicio_b3).toDate();
                  vm.fechaFinG= moment(vm.fechas_evaluaciones.fecha_fin_b3).toDate();
                  
                  console.log(bimestre);
              break;
              case "4":
                  vm.fechaInicioG= moment(vm.fechas_evaluaciones.fecha_inicio_b4).toDate();
                  vm.fechaFinG= moment(vm.fechas_evaluaciones.fecha_fin_b4).toDate();
                  
                  console.log(bimestre);
              break;
              case "5":
                  vm.fechaInicioG= moment(vm.fechas_evaluaciones.fecha_inicio_b5).toDate();
                  vm.fechaFinG= moment(vm.fechas_evaluaciones.fecha_fin_b5).toDate();
        ...
        function updateFechaEvalBim(){
          vm.flagI = false;
          vm.flagF = false;
          if(vm.fechaInicioG !==undefined && vm.fechaFinG!==undefined){
              var fechaIValida = moment([vm.fechaInicioG.getFullYear(),vm.fechaInicioG.getMonth(), vm.fechaInicioG.getDate()]);
              var fechaFValida = moment([vm.fechaFinG.getFullYear(),vm.fechaFinG.getMonth(), vm.fechaFinG.getDate()]);
             
             var fechaI = vm.fechaInicioG.getFullYear() + "-"+ ((vm.fechaInicioG.getMonth()+1 < 10)? "0": '')+(vm.fechaInicioG.getMonth()+1)+"-"+ ((vm.fechaInicioG.getDate() < 10)? "0": '')+(vm.fechaInicioG.getDate());
             var fechaF = vm.fechaFinG.getFullYear() + "-"+ ((vm.fechaFinG.getMonth()+1 < 10)? "0": '')+(vm.fechaFinG.getMonth()+1)+"-"+ ((vm.fechaFinG.getDate() < 10)? "0": '')+(vm.fechaFinG.getDate());
                  
              var fI = "";
              var fF = "";

                  if(fechaIValida.isValid() && fechaFValida.isValid()){

                      if(moment(fechaF).isSameOrBefore(fechaI)){
                          swal("Ups!", "No puede ser la misma o una fecha menor a la inicial!", "error");
                        ...
                        servicioevaluar.getFechasEval().then(function(data) {
                             var i=1;
                            vm.fEval = data[0];
                              angular.forEach(vm.fEval, function(date,key){

                                 fI =  moment(fechaI).isBetween(vm.fEval["fecha_inicio_b"+i], vm.fEval["fecha_fin_b"+i]);
                                 fF=  moment(fechaF).isBetween(vm.fEval["fecha_inicio_b"+i], vm.fEval["fecha_fin_b"+i]);
                            ...
                            if(fI && fF){
                                ...
                                swal("Ups!", "Ambas fechas se traslapan!", "error");
                                ...
                                if(!fI){
                                  if(!fF){
                                     ...
                                     servicioevaluar.updateFechaEvalBim(vm.bim,fechaI,fechaF).then(function(data) {
                                     swal("Exito!", "Actualizado correctamente!", "success");
                                     getFechasEvalBimestre(vm.bim);
                                     ...
                                     swal("Ups!", "Se traslapa la fecha final!", "error");
                                  }
                              }else{
                                 swal("Ups!", "Se traslapa la fecha de inicio!", "error");
                              }
                     ...
                     // moment('2010-10-20').isBetween('2010-10-19', '2010-10-25'); // true
                        ...
                        //console.log(fechaI.format('YYYY[-]MM[-]DD'));
                        //console.log(fechaF.format('YYYY[-]MM[-]DD'));
                        /*
                         servicioevaluar.updateFechaEvalBim(vm.bim,fechaI,fechaF).then(function(data) {
                      ...
                      swal("Ups!",'Fecha(s) invalida!', 'error');
      ...
      serviciodocente.getAlumnosByDocente(local.id_user).then(function(data) {
        ...
        var anioF= new Date(vm.datos_Curso_Actual.anioFin);
        var cabecera={
          grado: vm.datos_grupo_docente[0].grado,
          grupo: vm.datos_grupo_docente[0].nombre,
          ciclo: anioI.getFullYear().toString() + "-" + anioF.getFullYear().toString(),
          bimestre: vm.bim,
          profesor:local.nombre+" "+local.apPaterno+" "+local.apMaterno,

        }
 
        pdf(vm.evaluacioness, cabecera);
 
      })
 
    }
 
     function pdf(arr, cabe) {
      var listaCalf = [];
      for (var i = 0; i < arr.length; i++) {
        var row = [{
          ...
          text: arr[i].nombre_completo,
          alignment: 'center',
          border: [true, false, true, true]
        }];
        for(var j = 0; j < arr[i].datosMateria.califMateria.length; j++){
 
          row.push({
                        text: arr[i].datosMateria.califMateria[j],
                        alignment: 'center',
                        border: [true, false, true, true]
                        })
          //holaaaa
...
//////////////////////////////////////////////////////////////////////////////
      ...
      var colum=[
      {
        ...
        for(var k = 0; k < arr[0].datosMateria.nombreMateria.length; k++){
 
          colum.push({
                        text: arr[0].datosMateria.nombreMateria[k],
                        alignment: 'center',
                        border: [true, true, true, true]
                        })
        }
        listaMaterias.push(colum);
            ...
            table: {
              ...
              body: [
                ...
                [{
                  ...
                  text: ['ESCUELA PRIMARIA ROSARIO CASTELLANOS\n CLAVE:20DPR3331V\n',{text: cabe.grado},'° GRADO GRUPO: ',{text: cabe.grupo},'\nCICLO ESCOLAR: ',{text: cabe.ciclo},'\nRESULTADOS DE LA EVALUACION: BIMESTRE ',{text: cabe.bimestre},'\nPROFESOR: ',{text: cabe.profesor}],
                  alignment: 'center',
                  border: [false, false, false, false],
                  fontSize: 12,
                }, {
 ...
 /* function getMateriasByAlumno() {
            serviciomateria.getMateriasByAlumno(local.id_user).then(function(data) {
                vm.datos_materias_alumno = data;
                console.log("materias del alumno");
                console.log(vm.datos_materias_alumno);
            }).catch(function(err) {});
        }*/

       /* function califBim1Grado1() {
            serviciomateria.califBim1Grado1(local.id_user, 1).then(function(data) {
                vm.datos_bim1_g1 = data;
                console.log("calificaciones b1");
                console.log(vm.datos_bim1_g1);
            }).catch(function(err) {});
        }*/
            ...
            /*function verRegistro(){
              ...
              //{}
              for(var i=0; i<vm.registro.length;i++){
                for(var j=0; j<vm.alumnoEvaluar.length; j++){
                  if(vm.registro[i].id===vm.alumnoEvaluar[j].idAlumno){
                      
                      vm.alumnoEvaluar[j].idEA=vm.registro[i].cal1;
                      vm.alumnoEvaluar[i].idEF=vm.registro[i].cal2;
                      vm.alumnoEvaluar[i].idEsp=vm.registro[i].cal3;
                      vm.alumnoEvaluar[i].idENS=vm.registro[i].cal4;
                      vm.alumnoEvaluar[i].idFCE=vm.registro[i].cal5;
                      vm.alumnoEvaluar[i].idMat=vm.registro[i].cal6;

                      vm.alumnoEvaluar[i].idDocente=local.id_user;
                      vm.alumnoEvaluar[i].idBimestre=vm.bim;
                      vm.alumnoEvaluar[i].idCurso=vm.datos_Curso_Actual.idCurso;
                      //vm.alumnoEvaluar[i].idGrupo = vm.datos_grupo_docente[0].idGrupo;
                      var p = vm.registro[i].cal1+vm.registro[i].cal2+vm.registro[i].cal3+vm.registro[i].cal4+vm.registro[i].cal5+vm.registro[i].cal6;
                      //vm.alumnoEvaluar[i].promedio =vm.registro[i].cal1+vm.registro[i].cal2+vm.registro[i].cal3+vm.registro[i].cal4+vm.registro[i].cal5+vm.registro[i].cal6;
                      vm.alumnoEvaluar[i].promedio= (p/6).toFixed(2);
                  }
                }
              }
              serviciogrupo.guardarPrimerGrado(vm.alumnoEvaluar).then(function(data) {
                console.log("ARReNVIAR")
              console.log(vm.alumnoEvaluar);
            }).catch(function(err) {});
