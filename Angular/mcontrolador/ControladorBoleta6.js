(function() {
    angular.module("primariaAngular").controller("BoletaseisCtrl", BoletaseisController);

    function BoletaseisController($http, $scope, Flash, $auth,validaciones,toastr,   servicioevaluar, Upload, serviciogrupo, servicioalumno, serviciocurso,serviciocambio,serviciofolio) {
        var vm = this;
        vm.llena=llena;
        ...
        function getEvaluarFolio($event){
            var msj=validaciones.foliovalido(vm.folio);
            console.log(vm.folio);
            if(msj){
            serviciofolio.evaluarfolio(vm.folio).then(function(data){
                vm.folio=" ";
                swal("Ups!", "Este folio no cumple con el formato requerido: [A-Z|0-9]", "error");
        ...
        function getAlumnosByDocente() {
          ...
          function promedioNivel(){
            servicioevaluar.promedioFinalNivel(vm.alumnoSelect).then(function(data){
                vm.nivel=data;
...
function llena(){
    var anioI= new Date(vm.cabecera[0].anioInicio);
            ...
            var ni=parseFloat(vm.nivel[0].final);
            var nive=ni.toFixed(1);
...
var dd = {
    ...
    content: [
        ...
        columns:[
                ...
                {
                    margin: [ -30, 0, -50, 0 ],
                    text: ['6° GRADO DE EDUCACIÓN PRIMARIA   CICLO ESCOLAR ',{text:datos.ciclo}], fontSize:8,  style: 'tableHeader', border: [false, false, false, false], alignment: 'center'
                }
                ],
                //imagen centenario
                [{ 
